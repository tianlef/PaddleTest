...
(python paddlemix/examples/cogvlm/chat_demo.py \
    --model_name_or_path "THUDM/cogvlm-chat") 2>&1 | tee ${log_dir}/cogvlm_infer.log
tmp_exit_code=${PIPESTATUS[0]}
exit_code=$(($exit_code + ${tmp_exit_code}))
if [ ${tmp_exit_code} -eq 0 ]; then
    echo "cogvlm_infer run success" >>"${log_dir}/ce_res.log"
else
    echo "cogvlm_infer run fail" >>"${log_dir}/ce_res.log"
fi
