...
case_name=single_graph_inference
echo "******* ${model_name}_${case_name} begin***********"
(python paddlemix/examples/minicpm-v-2_6/single_image_infer.py) 2>&1 | tee ${log_dir}/${model_name}_${case_name}.log
...
case_name=video_inference
echo "******* ${model_name}_${case_name} begin***********"
(python paddlemix/examples/minicpm-v-2_6/video_infer.py) 2>&1 | tee ${log_dir}/${model_name}_${case_name}.log
tmp_exit_code=${PIPESTATUS[0]}
exit_code=$(($exit_code + ${tmp_exit_code}))
if [ ${tmp_exit_code} -eq 0 ]; then
    echo "${model_name}_${case_name} run success" >>"${log_dir}/ce_res.log"
