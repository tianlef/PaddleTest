...
(python run_predict.py \
  ...
  --input_audio https://paddlenlp.bj.bcebos.com/models/community/paddlemix/audio-files/wave.wav) 2>&1 | tee ${log_dir}/imagebind_predict.log
tmp_exit_code=${PIPESTATUS[0]}
exit_code=$(($exit_code + ${tmp_exit_code}))
if [ ${tmp_exit_code} -eq 0 ]; then
  echo "imagebind_predict run success" >>"${log_dir}/ce_res.log"
else
  echo "imagebind_predict run fail" >>"${log_dir}/ce_res.log"
fi
