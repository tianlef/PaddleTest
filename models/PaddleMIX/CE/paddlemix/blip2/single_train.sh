...
(CUDA_VISIBLE_DEVICES=0 python paddlemix/examples/blip2/run_pretrain_stage1.py \
    ...
    --warmup_steps 100 \
    --max_steps 100 ) 2>&1 | tee ${log_dir}/blip2_single_run_pretrain_stage1.log
tmp_exit_code=${PIPESTATUS[0]}
exit_code=$(($exit_code + ${tmp_exit_code}))
if [ ${tmp_exit_code} -eq 0 ]; then
    echo "blip2_single_run_pretrain_stage1 run success" >>"${log_dir}/ce_res.log"
else
    echo "blip2_single_run_pretrain_stage1 run fail" >>"${log_dir}/ce_res.log"
fi
...
(CUDA_VISIBLE_DEVICES=0 python paddlemix/examples/blip2/run_pretrain_stage2.py \
    --per_device_train_batch_size 64 \
    --warmup_steps 100 \
    --max_steps 100) 2>&1 | tee ${log_dir}/blip2_single_run_pretrain_stage2.log
tmp_exit_code=${PIPESTATUS[0]}
exit_code=$(($exit_code + ${tmp_exit_code}))
if [ ${tmp_exit_code} -eq 0 ]; then
    echo "blip2_single_run_pretrain_stage2 run success" >>"${log_dir}/ce_res.log"
else
    echo "blip2_single_run_pretrain_stage2 run fail" >>"${log_dir}/ce_res.log"
fi
