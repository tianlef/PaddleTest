...
if [ ${tmp_exit_code} -eq 0 ]; then
    echo "blip2_single_run_eval_vqav2_zeroshott run success" >>"${log_dir}/ce_res.log"
else
    echo "blip2_single_run_eval_vqav2_zeroshot run fail" >>"${log_dir}/ce_res.log"
fi
...
exit_code=$(($exit_code + ${tmp_exit_code}))
if [ ${tmp_exit_code} -eq 0 ]; then
    echo "blip2_single_run_eval_caption run success" >>"${log_dir}/ce_res.log"
else
    echo "blip2_single_run_eval_caption run fail" >>"${log_dir}/ce_res.log"
fi
echo "*******blip2_single_run_eval_caption end***********"
unset http_proxy
unset https_proxy
