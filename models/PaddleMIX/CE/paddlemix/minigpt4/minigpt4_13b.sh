...
(python run_predict.py \
    ...
    --length_penalty 0.0 \
    --temperature 1.0) 2>&1 | tee ${log_dir}/minigpt4_13b_predict.log
tmp_exit_code=${PIPESTATUS[0]}
exit_code=$(($exit_code + ${tmp_exit_code}))
if [ ${tmp_exit_code} -eq 0 ]; then
    echo "minigpt4_13b_predict run success" >>"${log_dir}/ce_res.log"
else
    echo "minigpt4_13b_predict run fail" >>"${log_dir}/ce_res.log"
fi
